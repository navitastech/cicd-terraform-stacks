...
module "api_asg" {
  ...
  asg_name = "demo_api_${var.stack_name}"
  load_balancer_names =  "${module.api_elb.elb_name}"
...
module "api_asg_scale_up" {
  source = "../modules/tf_module_autoscale_policy"
  app_name = "${var.app_name}"
  app_env = "${var.app_env}"
  scale_type ="up"
  scale_adjustment = "1"
  asg_name = "${module.api_asg.asg_id}"
  aws_region = "${var.aws_region}"
}

module "api_asg_scale_down" {
  source = "../modules/tf_module_autoscale_policy"
  app_name = "${var.app_name}"
  app_env = "${var.app_env}"
  scale_type ="down"
...
module "api_cpu_high_alarm" {
  ...
  statistic = "Average"
  threshold = "65"
  autoscaling_group_name = "${module.api_asg.asg_name}"
  alarm_description ="scale up if CPU utlization is > 65% over 3 minutes"
  alarm_actions = "${module.api_asg_scale_up.scale_policy_arn}"
  aws_region = "${var.aws_region}"
}

module "api_cpu_low_alarm" {
  source = "../modules/tf_module_cloudwatch_alarm"
 is_create_resource = "0"
  app_name = "${var.app_name}"
  app_env = "${var.app_env}"
  ...
  alarm_actions = "${module.api_asg_scale_down.scale_policy_arn}"
  aws_region = "${var.aws_region}"
